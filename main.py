    ...
    def __init__(self, name: str, side: str):
        ...
        for i in range(self.side + 1):
            for j in range(self.side + 1):
                self.cells[i][j].config(bg='#333' if (i != self.side) ^ (j != self.side) else '#000', fg='White')
                self.cells[i][j].grid(row=i, column=j)
        if os.path.exists('results.csv'):
            with open('results.csv') as file:
                results = [row for row in file.read().split('\n')[1:-1] if int(row.split(',')[1]) == self.side]
                self.record = max(results, key=lambda row: (int(row.split(',')[0]), -results.index(row)))
                self.cells[0][-1].config(text='Leader:', fg='#888')
                self.cells[1][-1].config(text=f'{self.record.split(",")[2]}\n{self.record.split(",")[0]}', fg='#000')
